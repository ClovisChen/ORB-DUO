...
bool LoopClosing::DetectLoop()
{
    ...
    /// 步骤3：在所有关键帧中找出闭环备选帧(vpLoopCandidates)
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            if(bConsistent)/// 如果"该子候选组"与该"子连续组"相连
            {
                int nPreviousConsistency = mvConsistentGroups[iG].second;
                int nCurrentConsistency = nPreviousConsistency + 1;
                if(!vbConsistentGroup[i])/// @Clovis IG->i
...
bool LoopClosing::ComputeSim3()
{
    ...
    /// 一直循环所有的候选帧，每个候选帧Ransac迭代5次，如果5次迭代后得不到结果，就换下一个候选帧
...
void LoopClosing::CorrectLoop()
{
    ...
    /// @Clovis (wubo:这两句话应该放在OptimizeEssentialGraph之前),我认为没有问题
